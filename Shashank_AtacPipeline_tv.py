...
class Pipeline():
    ...
    def trim_reads(self,fastq_path):
        '''
        trim reads with trimgalore
        '''
        self.concat_path = fastq_path
        if not os.path.exists(self.data_path + '/trimmed_fastqs'):
            os.makedirs(self.data_path + '/trimmed_fastqs')
        for index, sample in self.ssheet.iterrows():
            core_id = sample['SampleName']
            cmd = f'''#!/usr/bin/bash
#SBATCH --nodes=1
#SBATCH --mem=16gb
#SBATCH --ntasks=16
...
FastqFiles=( $( ls {core_id}_*R1*fastq.gz) )

for r1 in ${{FastqFiles[@]}}; do
    r2=${{r1/R1/R2}}
    trim_galore --paired --length 24 --stringency 3 --fastqc $r1 $r2 
done
        '''
            f = open(f'{self.submission_path}/trim_{core_id}.sh','w+')
            f.write(cmd)
            f.close()
        return None
...
#SBATCH --nodes=1
#SBATCH --mem=16gb
#SBATCH --output={self.output_file_dir}/align_{sampleID}.out
#SBATCH --account=scripps-dept
#SBATCH --qos=scripps-dept
...
cd {self.bam_dir}
QualFilt={core_id}_sorted.bam
fixmate={core_id}_fixmate.bam
fixsorted={core_id}_fixsorted.bam
dupremoved={core_id}_duprem.bam
module load samtools
samtools view -f 2 -q 20 -h {core_id}.bam | samtools sort -n -o $QualFilt -O bam 
samtools fixmate -m $QualFilt $fixmate
samtools sort -O bam -o $fixsorted $fixmate 
samtools markdup -r $fixsorted $dupremoved
rm $QualFilt $fixmate $fixsorted 
'''
            f = open(f'{self.submission_path}/remove_dups_{core_id}.sh','w+')
            f.write(cmd)
            f.close()
        return None

    def macs_peaks_broad(self,peaks_dir):
        self.peaks_dir = peaks_dir
        if not os.path.exists(self.peaks_dir):
            os.makedirs(self.peaks_dir)
        for index, sample in self.ssheet.iterrows():
            core_id = sample["SampleName"]
            cmd = f'''#!/usr/bin/bash
#SBATCH --nodes=1
...
duprembam={core_id}_duprem.bam
macsname={core_id}_MACS
module load macs/2.1.0
cd {self.bam_dir}

macs2 callpeak -t $duprembam --broad -f BAMPE -g mm -n {self.peaks_dir}/$macsname -q 0.05 -B
'''
            f = open(f'{self.submission_path}/MACS_broad_{core_id}.sh','w+')
            f.write(cmd)
            f.close()
        return None
    
    def pileup_toBw(self):
        for index, sample in self.ssheet.iterrows():
...
awk '{{print $1"\t"$2"\t"$3"\t"$4}}' {core_id}_*broadPeak* > cropped
sort -k1,1 -k2,2n cropped > cropped_sorted
bedGraphToBigWig cropped_sorted RefSeq_mm39_GenomeSizes.txt {core_id}_broadPeaks.bw
'''
            f = open(f'{self.submission_path}/BdgToBw_{core_id}.sh','w+')
            f.write(cmd)
            f.close()
        
        return None

    def macs_peaks_narrow(self,peaks_dir):
        self.peaks_dir = peaks_dir
        if not os.path.exists(self.peaks_dir):
            os.makedirs(self.peaks_dir)
        for index, sample in self.ssheet.iterrows():
...
macs2 callpeak -t $duprembam -f BAMPE -g mm --nomodel --shift 100 --extsize 200 -n {self.peaks_dir}/$macsname -q 0.05 -B
'''
            f = open(f'{self.submission_path}/MACS_narrow_{core_id}.sh','w+')
            f.write(cmd)
            f.close()
